    ...
    <div className="min-vh-100 d-flex position-relative overflow-hidden bg-white text-dark">
      ...
      <div className="position-absolute w-100 h-100 pointer-events-none" style={{ zIndex: 0, overflow: 'hidden' }}>
        <motion.div 
          ...
          style={{ width: '600px', height: '600px', filter: 'blur(150px)', opacity: 0.1, top: '-10%', left: '-10%' }}
        />
        <motion.div 
          animate={{ x: [0, -100, 0], y: [0, -50, 0], scale: [1, 1.5, 1] }} 
          transition={{ duration: 20, repeat: Infinity, ease: 'easeInOut' }}
          className="position-absolute bg-info rounded-circle" 
          style={{ width: '500px', height: '500px', filter: 'blur(150px)', opacity: 0.1, bottom: '-10%', right: '-10%' }}
        />
      </div>
      ...
      <div className="container-fluid m-0 p-0 d-flex w-100 position-relative z-1">
        <div className="row g-0 w-100 flex-grow-1">
          ...
          <div className="col-lg-5 d-none d-lg-flex position-relative overflow-hidden p-0" style={{ borderRight: '1px solid rgba(0,0,0,0.05)' }}>
            ...
            <div className="position-absolute w-100 h-100" style={{ background: 'linear-gradient(to right, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.4) 100%)', zIndex: 1 }}></div>
            ...
            <div className="position-relative z-2 d-flex flex-column justify-content-center p-5 h-100 text-white">
              ...
              >
                ...
                <p className="text-white-50 fs-5" style={{ maxWidth: '400px' }}>
                  Set up your multi-tenant warehouse, billing currency, and global admin settings in less than 2 minutes.
                </p>
              </motion.div>
            </div>
          </div>
          ...
          <div className="col-lg-7 d-flex align-items-center justify-content-center p-4 p-sm-5" style={{ overflowY: 'auto' }}>
            <motion.div 
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6 }}
              className="w-100" style={{ maxWidth: '650px' }}
            >
              <div className="glass-card rounded-4 shadow-lg p-4 p-md-5" style={{ background: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(20px)', border: '1px solid rgba(0,0,0,0.08)' }}>
                <div className="text-center mb-4">
                  <h3 className="fw-bold text-dark mb-2">Create Workspace</h3>
                  <p className="text-muted fs-7">Follow the steps to establish your cloud operations</p>
                </div>
                ...
                <div className="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom border-light-subtle">
                  {STEPS.map((s) => {
                    ...
                    return (
                      <div key={s.id} className="d-flex flex-column align-items-center gap-1 flex-grow-1" style={{ opacity: isActive || isPast ? 1 : 0.4 }}>
                        <div className={`rounded-circle d-flex align-items-center justify-content-center border ${isActive ? 'border-primary bg-primary text-white fw-bold' : isPast ? 'border-success bg-success text-white' : 'border-secondary text-muted'}`} style={{ width: '28px', height: '28px', fontSize: '0.75rem' }}>
                          {isPast ? '✓' : s.id}
                        </div>
                        <span className="d-none d-md-inline" style={{ fontSize: '0.65rem', fontWeight: isActive ? 600 : 400 }}>{s.title}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="bg-light mb-4" style={{ height: '4px', borderRadius: '2px' }}>
                  <motion.div 
                  ...
                  >
                    ...
                    {step === 1 && (
                      <div className="flex-grow-1">
                        <h5 className="fw-bold mb-3 text-dark">Company Details</h5>
                        <div className="row g-3">
                          ...
                          <div className="col-md-6">
                            <label className="form-label text-muted fs-7">GST / Tax ID (Optional)</label>
                            <input type="text" className="form-control bg-white border-light-subtle text-dark" name="gstNumber" value={formData.gstNumber} onChange={handleChange} placeholder="Optional" />
                          </div>
                          <div className="col-md-6">
                            <label className="form-label text-muted fs-7">Phone Number (Optional)</label>
                            <input type="text" className="form-control bg-white border-light-subtle text-dark" name="phone" value={formData.phone} onChange={handleChange} placeholder="Optional" />
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {step === 2 && (
                      <div className="flex-grow-1">
                        <h5 className="fw-bold mb-3 text-dark">Administrator Settings</h5>
                        <div className="row g-3">
                          <div className="col-12">
                          ...
                          <div className="col-12">
                            ...
                            <input type="email" className="form-control bg-white border-light-subtle text-dark" name="adminEmail" value={formData.adminEmail} onChange={handleChange} placeholder="john@example.com" />
                          </div>
                          <div className="col-12">
                          ...
                          <div className="col-md-6">
                            ...
                            <input type="password" className="form-control bg-white border-light-subtle text-dark" name="adminPassword" value={formData.adminPassword} onChange={handleChange} placeholder="••••••••" />
                          </div>
                          <div className="col-md-6">
                            <label className="form-label text-muted fs-7">Confirm Password</label>
                            <input type="password" className="form-control bg-white border-light-subtle text-dark" name="adminConfirmPassword" value={formData.adminConfirmPassword} onChange={handleChange} placeholder="••••••••" />
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {step === 3 && (
                      <div className="flex-grow-1">
                        <h5 className="fw-bold mb-3 text-dark">Initial Warehouse Config</h5>
                        <div className="row g-3">
                          <div className="col-md-12">
                            <label className="form-label text-muted fs-7">Warehouse Code (e.g. MAIN)</label>
                    ...
                    {step === 4 && (
                      <div className="flex-grow-1">
                        <h5 className="fw-bold mb-3 text-dark">Select Subscription Plan</h5>
                            ...
                            >
                              <div className="d-flex justify-content-between align-items-center mb-1">
                                <span className="fw-bold text-dark d-flex align-items-center gap-2">
                                  {plan.name}
                                  {plan.popular && <span className="badge bg-primary text-white rounded-pill fs-8">POPULAR</span>}
                                </span>
                    ...
                    {step === 5 && (
                      <div className="flex-grow-1">
                        <h5 className="fw-bold mb-3 text-dark">Review Details</h5>
              ...
              <div className="text-center mt-4">
                <p className="text-muted fs-7">Already have an account? <Link to="/login" className="text-primary text-decoration-none hover-text-dark fw-semibold">Log in instead</Link></p>
              </div>
            </motion.div>
          </div>
